...
//2013-09-14
inline bool Generate::checkActivitiesSameRoomIfConsecutive(const QList<int>& globalConflActivities, int rm, int level, int ai, int d, int h, QList<int>& tmp_list)
{
	Q_UNUSED(level);
 ...
 int& roomSlot, int& selectedSlot, QList<int>& localConflActivities, bool& canBeUnspecifiedPreferredRoom, QList<int>& realRoomsList)
{
	...
	//qSort(allowedRoomsList); //To keep the generation identical on all computers - 2013-01-03
...
if(threaded){
		...
		//////////////care for students max days per week
		for(int i=0; i<gt.rules.nInternalSubgroups; i++)
			for(int j=0; j<gt.rules.nDaysPerWeek; j++)
					...
					if(necessary > item->durationOfActivitiesWithActivityTagForSubgroup){
						...
						for(;;){
							if(candidatesSet.count()==0){
								...
								int ai2;
								if(level>0){
									int q=randomKnuth(candidatesList.count());
					...
					while(target>0){
						...
						if(level>0){
							...
							for(int k : qAsConst(tmpSortedList)){
								QSet<int> acts=slotSetOfActivities[k];
								assert(acts.count()>0);
							...
							int q=randomKnuth(optCandidates.count());
