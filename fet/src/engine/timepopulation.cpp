...
void TimePopulation::init(Rules& r, int np)
{
...
int TimePopulation::populationNumber()
{
	return this->n;
}

void TimePopulation::makeTimesUnallocated(Rules& r)
{
	...
	for(int i=0; i < this->n; i++)
		this->chromosomes[i].makeTimesUnallocated(r);

	this->sorted=false;
	this->bestFirst=false;
}

void TimePopulation::makeTimesRandom(Rules& r)
{
	assert(this->initialized);
	assert(this->n>0);

	for(int i=0; i<this->n; i++)
		this->chromosomes[i].makeTimesRandom(r);

	this->sorted=false;
	this->bestFirst=false;
}

TimeChromosome& TimePopulation::bestChromosome(Rules& r)
{
	assert(this->initialized);
	...
	TimeChromosome* c=&chromosomes[0];
	int j=-1;
...
TimeChromosome& TimePopulation::medianChromosome(Rules& r)
{
	assert(this->initialized);
...
double TimePopulation::totalHardFitness(Rules& r)
{
	assert(this->initialized);
...
double TimePopulation::totalSoftFitness(Rules& r)
{
	assert(this->initialized);
...
void TimePopulation::read(Rules& r, QTextStream &tis)
{
	tis>>this->n;
	for(int i=0; i<this->n; i++)
...
void TimePopulation::write(Rules& r, QTextStream &tos)
{
