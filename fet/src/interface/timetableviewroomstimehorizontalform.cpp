...
TimetableViewRoomsTimeHorizontalForm::TimetableViewRoomsTimeHorizontalForm(QWidget* parent): QDialog(parent)
{
	...
	roomsSpaceAction=new QAction(tr("Rooms space", "View/edit the selected rooms' space constraints"), this);
	...
	constraintsMenu->addAction(roomsSpaceAction);
	...
	connect(roomsSpaceAction, &QAction::triggered, this, &TimetableViewRoomsTimeHorizontalForm::roomsSpace);
	connect(activitiesTimeAction, &QAction::triggered, this, &TimetableViewRoomsTimeHorizontalForm::activitiesTime);
	connect(activitiesSpaceAction, &QAction::triggered, this, &TimetableViewRoomsTimeHorizontalForm::activitiesSpace);
	connect(constraintsPushButton, &QPushButton::clicked, this, &TimetableViewRoomsTimeHorizontalForm::constraints);
...
void TimetableViewRoomsTimeHorizontalForm::constraints()
{
	constraintsMenu->popup(QCursor::pos());
}

void TimetableViewRoomsTimeHorizontalForm::roomsSpace()
{
	if(generation_running || generation_running_multi){
		QMessageBox::information(this, tr("FET information"),
			tr("Generation in progress. Please stop the generation before this."));
	...
	for(int t=0; t<gt.rules.nInternalRooms; t++){
		...
		for(int k=0; k<roomsTimetableTable->columnCount(); k++){
			if(roomsTimetableTable->item(t, k)->isSelected()){
				srm.append(t);
			}
		}
	}
	...
	if(srm.isEmpty()){
		QMessageBox::information(this, tr("FET information"), tr("Please select at least one room (one table cell)"));
		return;
	}

	QList<SpaceConstraint*> tscl;
	for(SpaceConstraint* sc : std::as_const(gt.rules.spaceConstraintsList)){
		for(int rm : std::as_const(srm)){
			if(sc->isRelatedToRoom(gt.rules.internalRoomsList[rm]->name)){
				tscl.append(sc);
				break;
			}
		}
	...
	QString s;
	if(srm.count()==1)
		s=tr("Room: %1").arg(gt.rules.internalRoomsList[srm.at(0)]->name);
	else
		s=tr("%1 rooms selected", "%1 is the number of selected rooms").arg(srm.count());
	ListOfRelatedSpaceConstraintsForm form(this, FILTER_IS_ROOM, QList<int>(), s, tscl);
	form.exec();
}

void TimetableViewRoomsTimeHorizontalForm::activitiesTime()
{
	if(!(students_schedule_ready && teachers_schedule_ready)){
		QMessageBox::warning(this, tr("FET warning"), tr("Timetable not available in view rooms timetable dialog - please generate a new timetable"));
		return;
...
void TimetableViewRoomsTimeHorizontalForm::activitiesSpace()
{
	if(!(students_schedule_ready && teachers_schedule_ready)){
		QMessageBox::warning(this, tr("FET warning"), tr("Timetable not available in view rooms timetable dialog - please generate a new timetable"));
		return;
