 ...
 ***************************************************************************
                          exportinterface.cpp  -  description
                             -------------------
    begin                : 2026
...
void Export::exportCSV(QWidget* parent){
	...
	if(!ok){
		...
		bool okat, okr, oks, okt, okst, okact, okacts, oktim, oktna;
		...
		oktna=exportCSVTeacherNotAvailable(newParent, lastWarnings, textquote, fieldSeparator, head, msgBoxButton);
		
		ok=okat && okr && oks && okt && okst && okact && okacts && oktim && oktna;
...
bool Export::exportCSVTeacherNotAvailable(QWidget* parent, QString& lastWarnings, const QString& textquote, const QString& fieldSeparator, const bool head, QMessageBox::StandardButton& msgBoxButton){
	QString s2=INPUT_FILENAME_XML.right(INPUT_FILENAME_XML.length()-INPUT_FILENAME_XML.lastIndexOf(FILE_SEP)-1);	//TODO: remove s2, because too long filenames!
