...
#include <algorithm> //for std::max and std::stable_sort
...
//const double INFINITE_FITNESS=1.0e9;
...
void TimetableGenerateMultipleForm::start(){
	...
	allTimetablesFitnessAndDescription.clear();
	haveHardConflicts=false;
	haveSoftConflicts=false;
...
void TimetableGenerateMultipleForm::timetableGenerated(int nThread, int timetable, const QString& description, bool ok)
{
	...
	if(nGeneratedTimetables<=nTimetables){
		...
		allTimetablesFitnessAndDescription.append(QPair<double, QString>(genMultiMatrix[nThread].c.conflictsTotal, s));
		if(ok){
			if(!haveSoftConflicts)
				haveSoftConflicts=true;
		}
		else{
			if(!haveHardConflicts)
				haveHardConflicts=true;
		}
		...
		if(ok){
			...
			conflictsString+="\n";
			conflictsString+=tr("Soft conflicts listing (in descending order):")+"\n";
...
void TimetableGenerateMultipleForm::stop()
{
	...
	if(nSuccessfullyGeneratedTimetables>=1){
		...
		for(int i=0; i<highestStageSolutions.count(); i++){
			...
			conflictsString+="\n";
			conflictsString+=tr("Conflicts listing (in descending order):")+"\n";
	...
	std::stable_sort(allTimetablesFitnessAndDescription.begin(), allTimetablesFitnessAndDescription.end(),
	 [](const QPair<double, QString>& a, const QPair<double, QString>& b){return a.first < b.first;});

	QString sortedTimetables=QString("");
	if(haveSoftConflicts && !haveHardConflicts){
		sortedTimetables=tr("The list of generated timetables, in ascending order of their total soft conflicts, is:")+QString("\n\n");
	}
	else if(!haveSoftConflicts && haveHardConflicts){
		sortedTimetables=tr("The list of generated timetables, in ascending order of their total conflicts, is:")+QString("\n\n");
	}
	else if(haveSoftConflicts && haveHardConflicts){
		sortedTimetables=tr("The list of generated timetables, in ascending order of their total (soft) conflicts, is:")+QString("\n\n");
	}
	else{
		assert(!haveSoftConflicts && !haveHardConflicts);
		sortedTimetables=tr("The list of generated timetables, in ascending order of their total conflicts, is:")+QString("\n\n");
	}

	for(const QPair<double, QString>& pr : std::as_const(allTimetablesFitnessAndDescription))
		sortedTimetables+=pr.second+QString("\n");

	if(nGeneratedTimetables>=1){
		TimetableExport::writeReportForMultiple(this, QString("\n")+ms3+stringForDisk+ms2, false);
		TimetableExport::writeReportSortedForMultiple(this, sortedTimetables+QString("\n")+ms3+stringForDisk+ms2);
	}
	else{
		TimetableExport::writeReportForMultiple(this, ms3+stringForDisk+ms2, true);
		TimetableExport::writeReportSortedForMultiple(this, sortedTimetables+ms3+stringForDisk+ms2);
	}
