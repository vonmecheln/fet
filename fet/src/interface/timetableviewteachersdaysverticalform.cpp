/***************************************************************************
                          timetableviewteachersdaysverticalform.cpp  -  description
                             -------------------
    begin                : 2024
...
void TimetableViewTeachersDaysVerticalDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
...
TimetableViewTeachersDaysVerticalForm::TimetableViewTeachersDaysVerticalForm(QWidget* parent): QDialog(parent)
{
	setupUi(this);
	...
	connect(closePushButton, &QPushButton::clicked, this, &TimetableViewTeachersDaysVerticalForm::close);
	connect(teachersListWidget, &QListWidget::currentTextChanged, this, &TimetableViewTeachersDaysVerticalForm::teacherChanged);
	connect(teachersTimetableTable, &QTableWidget::currentItemChanged, this, &TimetableViewTeachersDaysVerticalForm::currentItemChanged);
	...
	teacherTimeAction=new QAction(tr("Teacher time", "View/edit the selected teacher's time constraints"), this);
	teacherSpaceAction=new QAction(tr("Teacher space", "View/edit the selected teacher's space constraints"), this);
	...
	constraintsMenu->addAction(teacherTimeAction);
	constraintsMenu->addAction(teacherSpaceAction);
	...
	connect(teacherTimeAction, &QAction::triggered, this, &TimetableViewTeachersDaysVerticalForm::teacherTime);
	connect(teacherSpaceAction, &QAction::triggered, this, &TimetableViewTeachersDaysVerticalForm::teacherSpace);
	connect(activitiesTimeAction, &QAction::triggered, this, &TimetableViewTeachersDaysVerticalForm::activitiesTime);
	connect(activitiesSpaceAction, &QAction::triggered, this, &TimetableViewTeachersDaysVerticalForm::activitiesSpace);
	connect(constraintsPushButton, &QPushButton::clicked, this, &TimetableViewTeachersDaysVerticalForm::constraints);
...
///////////
	...
	if(!realView){
		teachersTimetableTable->setColumnCount(gt.rules.nHoursPerDay);
		teachersTimetableTable->setRowCount(gt.rules.nDaysPerWeek);

		for(int j=0; j<gt.rules.nDaysPerWeek; j++){
			QTableWidgetItem* item=new QTableWidgetItem(gt.rules.daysOfTheWeek[j]);
			teachersTimetableTable->setVerticalHeaderItem(j, item);
		}
		for(int i=0; i<gt.rules.nHoursPerDay; i++){
			QTableWidgetItem* item=new QTableWidgetItem(gt.rules.hoursOfTheDay[i]);
			teachersTimetableTable->setHorizontalHeaderItem(i, item);
		}
		...
		for(int j=0; j<gt.rules.nHoursPerDay; j++){
			for(int k=0; k<gt.rules.nDaysPerWeek; k++){
				...
				teachersTimetableTable->setItem(k, j, item);
		...
		teachersTimetableTable->setColumnCount(gt.rules.nRealHoursPerDay);
		teachersTimetableTable->setRowCount(gt.rules.nRealDaysPerWeek);
	...
	//added by Volker Dirr
	connect(&communicationSpinBox, &CommunicationSpinBox::valueChanged, this, &TimetableViewTeachersDaysVerticalForm::updateTeachersTimetableTable);
}

void TimetableViewTeachersDaysVerticalForm::newTimetableGenerated()
{
	/*setupUi(this);
...
*/
	...
	disconnect(teachersListWidget, &QListWidget::currentTextChanged, this, &TimetableViewTeachersDaysVerticalForm::teacherChanged);
	teachersListWidget->clear();
...
TimetableViewTeachersDaysVerticalForm::~TimetableViewTeachersDaysVerticalForm()
{
...
void TimetableViewTeachersDaysVerticalForm::resizeRowsAfterShow()
{
	teachersTimetableTable->resizeRowsToContents();
}

void TimetableViewTeachersDaysVerticalForm::teacherChanged(const QString& teacherName)
{
...
void TimetableViewTeachersDaysVerticalForm::updateTeachersTimetableTable(){
	if(!(students_schedule_ready && teachers_schedule_ready)){
		QMessageBox::warning(this, tr("FET warning"), tr("Timetable not available in view teachers timetable dialog - please generate a new timetable "
	...
	for(int j=0; j<(normalView?gt.rules.nHoursPerDay:gt.rules.nRealHoursPerDay) && j<teachersTimetableTable->columnCount(); j++){
		...
		for(int k=0; k<(normalView?gt.rules.nDaysPerWeek:gt.rules.nRealDaysPerWeek) && k<teachersTimetableTable->rowCount(); k++){
			int kk;
			if(normalView)
				kk=k;
			else
			...
			QFont font(teachersTimetableTable->item(k, j)->font());
			font.setBold(false);
			font.setItalic(false);
			font.setUnderline(false);
			teachersTimetableTable->item(k, j)->setFont(font);
			...
			if(ai!=UNALLOCATED_ACTIVITY){
				...
				if(colorsCheckBox->isChecked() /*USE_GUI_COLORS*/ /*&& act->studentsNames.count()>0*/){
					...
					if (brightness<0.5)
						teachersTimetableTable->item(k, j)->setForeground(QBrush(QColorConstants::White));
					else
						teachersTimetableTable->item(k, j)->setForeground(QBrush(QColorConstants::Black));
				}
				// add colors (end)
				//end by Marco Vassura
			}
			...
			teachersTimetableTable->item(k, j)->setText(s);
			teachersTimetableTable->item(k, j)->setToolTip(longString);
		}
	}
...
void TimetableViewTeachersDaysVerticalForm::resizeEvent(QResizeEvent* event)
{
...
QColor TimetableViewTeachersDaysVerticalForm::stringToColor(const QString& s)
{
...
void TimetableViewTeachersDaysVerticalForm::currentItemChanged(QTableWidgetItem* current, QTableWidgetItem* previous)
{
	Q_UNUSED(previous);
...
void TimetableViewTeachersDaysVerticalForm::lockTime()
{
...
void TimetableViewTeachersDaysVerticalForm::lockTimeSpace()
{
	this->lock(true, true);
}
...
void TimetableViewTeachersDaysVerticalForm::lock(bool lockTime, bool lockSpace)
{
	...
	if(addedT>0 || addedS>0 || unlockedT>0 || unlockedS>0)
		gt.rules.addUndoPoint(tr("Toggled lock/unlock a selection of activities in the"
		  " timetable view teachers days vertical dialog. The summary of the added/removed locking constraints is:\n\n%1").arg(s)+QString("\n"));
...
void TimetableViewTeachersDaysVerticalForm::help()
{
	QString s="";
...
void TimetableViewTeachersDaysVerticalForm::constraints()
{
