...
HelpTipsForm::HelpTipsForm()
{
	...
	s+=tr("1) Say you add a split activity or you add a constraint min n days between activities (say na activities),"
	...
	s+=tr("2) When adding constraints activities same starting time (or day), please remove redundant min n days constraints. "
	 "For instance, A1 (at same time with B1 and C1), A2 (same time with B2 and C2) and A3 (same time with B3 and C3)."
	 " You will normally have 3 or 6 constraints min n days between activities (first one or two with A1,A2,A3 and then "
	 "with B1,B2,B3 and then with C1,C2,C3). These 3 or 6 constraints min n days will result in a much stronger constraint, "
	...
	s+="\n\n";
	s+=tr("The correct way is to leave only the constraints min n days referring to the first group of activities (A)."
	...
	s+="\n\n";
	s+=tr("More details: the combination of 2 or more redundant constraints min n days between activities gives another "
	...
	s+="\n\n";
	s+=tr("It is IMPORTANT to remove redundant min n days constraints after you inputted data and before generating "
	 "(for instance, apply this after adding more constraints of type same starting time/day and before generating)."
	 " Any modification of the min n days constraints should be followed by this removal of redundant min n days "
	...
	s+="\n\n";
	s+=tr("Note: redundant constraints min n days with weight 100.0% do not really affect the generation. This is because the resultant of more constraints"
