/***************************************************************************
                          timetableviewstudentsdaysverticalform.cpp  -  description
                             -------------------
    begin                : 2024
...
void TimetableViewStudentsDaysVerticalDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
...
TimetableViewStudentsDaysVerticalForm::TimetableViewStudentsDaysVerticalForm(QWidget* parent): QDialog(parent)
{
	setupUi(this);
	...
	connect(closePushButton, &QPushButton::clicked, this, &TimetableViewStudentsDaysVerticalForm::close);
	connect(studentsTimetableTable, &QTableWidget::currentItemChanged, this, &TimetableViewStudentsDaysVerticalForm::currentItemChanged);
	...
	connect(teachersCheckBox, &QCheckBox::toggled, this, &TimetableViewStudentsDaysVerticalForm::updateStudentsTimetableTable);
	connect(studentsCheckBox, &QCheckBox::toggled, this, &TimetableViewStudentsDaysVerticalForm::updateStudentsTimetableTable);
	connect(subjectsCheckBox, &QCheckBox::toggled, this, &TimetableViewStudentsDaysVerticalForm::updateStudentsTimetableTable);
	connect(activityTagsCheckBox, &QCheckBox::toggled, this, &TimetableViewStudentsDaysVerticalForm::updateStudentsTimetableTable);
	connect(roomsCheckBox, &QCheckBox::toggled, this, &TimetableViewStudentsDaysVerticalForm::updateStudentsTimetableTable);

	connect(colorsCheckBox, &QCheckBox::toggled, this, &TimetableViewStudentsDaysVerticalForm::updateStudentsTimetableTable);

	connect(spanCheckBox, &QCheckBox::toggled, this, &TimetableViewStudentsDaysVerticalForm::updateStudentsTimetableTable);
...
//////////
	...
	if(!realView){
		studentsTimetableTable->setColumnCount(gt.rules.nHoursPerDay);
		studentsTimetableTable->setRowCount(gt.rules.nDaysPerWeek);
		for(int j=0; j<gt.rules.nDaysPerWeek; j++){
			QTableWidgetItem* item=new QTableWidgetItem(gt.rules.daysOfTheWeek[j]);
			studentsTimetableTable->setVerticalHeaderItem(j, item);
		}
		for(int i=0; i<gt.rules.nHoursPerDay; i++){
			QTableWidgetItem* item=new QTableWidgetItem(gt.rules.hoursOfTheDay[i]);
			studentsTimetableTable->setHorizontalHeaderItem(i, item);
		}
		for(int j=0; j<gt.rules.nHoursPerDay; j++){
			for(int k=0; k<gt.rules.nDaysPerWeek; k++){
				...
				studentsTimetableTable->setItem(k, j, item);
		...
		studentsTimetableTable->setColumnCount(gt.rules.nRealHoursPerDay);
		studentsTimetableTable->setRowCount(gt.rules.nRealDaysPerWeek);
	...
	newItemDelegate=new TimetableViewStudentsDaysVerticalDelegate(nullptr, studentsTimetableTable, studentsTimetableTable->rowCount(), gt.rules.nHoursPerDay);
	...
	connect(shownComboBox, qOverload<int>(&QComboBox::currentIndexChanged), this, &TimetableViewStudentsDaysVerticalForm::shownComboBoxChanged);

	//added by Volker Dirr
	connect(&communicationSpinBox, &CommunicationSpinBox::valueChanged, this, &TimetableViewStudentsDaysVerticalForm::updateStudentsTimetableTable);

	shownComboBoxChanged();
}

void TimetableViewStudentsDaysVerticalForm::newTimetableGenerated()
{
	/*setupUi(this);
...
void TimetableViewStudentsDaysVerticalForm::resizeRowsAfterShow()
{
	studentsTimetableTable->resizeRowsToContents();
}

void TimetableViewStudentsDaysVerticalForm::shownComboBoxChanged()
{
...
void TimetableViewStudentsDaysVerticalForm::yearChanged(const QString& yearName)
{
...
void TimetableViewStudentsDaysVerticalForm::groupChanged(const QString& groupName)
{
	if(!(students_schedule_ready && teachers_schedule_ready)){
		QMessageBox::warning(this, tr("FET warning"), tr("Timetable not available in view students timetable dialog - please generate a new timetable"));
		return;
...
void TimetableViewStudentsDaysVerticalForm::subgroupChanged(const QString& subgroupName)
{
...
void TimetableViewStudentsDaysVerticalForm::updateStudentsTimetableTable(){
	if(!(students_schedule_ready && teachers_schedule_ready)){
		QMessageBox::warning(this, tr("FET warning"), tr("Timetable not available in view students timetable dialog - please generate a new timetable "
	...
	for(int j=0; j<(normalView?gt.rules.nHoursPerDay:gt.rules.nRealHoursPerDay) && j<studentsTimetableTable->columnCount(); j++){
		...
		for(int k=0; k<(normalView?gt.rules.nDaysPerWeek:gt.rules.nRealDaysPerWeek) && k<studentsTimetableTable->rowCount(); k++){
			...
			QFont font(studentsTimetableTable->item(k, j)->font());
			font.setBold(false);
			font.setItalic(false);
			font.setUnderline(false);
			studentsTimetableTable->item(k, j)->setFont(font);

			//begin by Marco Vassura
			// add colors (start)
			//if(USE_GUI_COLORS) {
			studentsTimetableTable->item(k, j)->setBackground(studentsTimetableTable->palette().base());
			studentsTimetableTable->item(k, j)->setForeground(studentsTimetableTable->palette().text());
			//}
			// add colors (end)
			//end by Marco Vassura
			s="";
			...
			if(ai!=UNALLOCATED_ACTIVITY){
				...
				if(best_solution.times[ai]/gt.rules.nDaysPerWeek==jj){
					...
					if(spanCheckBox->isChecked() && act->duration>=2)
						studentsTimetableTable->setSpan(k, j, 1, act->duration);
				}
			...
			studentsTimetableTable->item(k, j)->setText(s);
			studentsTimetableTable->item(k, j)->setToolTip(longString);
		}
	}
...
QColor TimetableViewStudentsDaysVerticalForm::stringToColor(const QString& s)
{
...
void TimetableViewStudentsDaysVerticalForm::resizeEvent(QResizeEvent* event)
{
...
void TimetableViewStudentsDaysVerticalForm::currentItemChanged(QTableWidgetItem* current, QTableWidgetItem* previous)
{
	Q_UNUSED(previous);
...
void TimetableViewStudentsDaysVerticalForm::detailActivity(QTableWidgetItem* item)
{
	if(item==nullptr){
	...
	if(j>=0 && k>=0){
		...
		if(ai!=UNALLOCATED_ACTIVITY){
			...
			if(subgroupNotAvailableDayHour[i][kk][jj]){
				s2+=tr("Students subgroup is not available 100% in this slot");
				s2+="\n";
			}
...
void TimetableViewStudentsDaysVerticalForm::lockTime()
{
	lock(true, false);
}

void TimetableViewStudentsDaysVerticalForm::lockSpace()
{
	lock(false, true);
}

void TimetableViewStudentsDaysVerticalForm::lockTimeSpace()
{
	lock(true, true);
}

void TimetableViewStudentsDaysVerticalForm::lock(bool lockTime, bool lockSpace)
{
	...
	if(addedT>0 || addedS>0 || unlockedT>0 || unlockedS>0)
		gt.rules.addUndoPoint(tr("Toggled lock/unlock a selection of activities in the"
		  " timetable view students days vertical dialog. The summary of the added/removed locking constraints is:\n\n%1").arg(s)+QString("\n"));
...
void TimetableViewStudentsDaysVerticalForm::help()
{
	QString s="";
...
void TimetableViewStudentsDaysVerticalForm::studentsSubgroupTime()
{
	...
	if(gt.rules.correspondingRealStudentsSetName.contains(subgroupname))
		...
		realStudentsSetName=subgroupname;

	QList<TimeConstraint*> ttcl;
	for(TimeConstraint* tc : std::as_const(gt.rules.timeConstraintsList))
		if(tc->isRelatedToStudentsSet(gt.rules, realStudentsSetName))
			ttcl.append(tc);

	ListOfRelatedTimeConstraintsForm form(this, FILTER_IS_STUDENTS_SET, QList<int>(), tr("Students subgroup: %1").arg(subgroupname), ttcl);
	form.exec();
}
...
void TimetableViewStudentsDaysVerticalForm::studentsSubgroupSpace()
{
	...
	for(SpaceConstraint* sc : std::as_const(gt.rules.spaceConstraintsList))
		if(sc->isRelatedToStudentsSet(gt.rules, realStudentsSetName))
			tscl.append(sc);

	ListOfRelatedSpaceConstraintsForm form(this, FILTER_IS_STUDENTS_SET, QList<int>(), tr("Students subgroup: %1").arg(subgroupname), tscl);
	form.exec();
}
