/***************************************************************************
                          addconstraintteachersmaxroomchangesperdayinintervalform.cpp  -  description
                             -------------------
    begin                : 2024
...
AddConstraintTeachersMaxRoomChangesPerDayInIntervalForm::AddConstraintTeachersMaxRoomChangesPerDayInIntervalForm(QWidget* parent): QDialog(parent)
{
	setupUi(this);

	addConstraintPushButton->setDefault(true);

	connect(addConstraintPushButton, &QPushButton::clicked, this, &AddConstraintTeachersMaxRoomChangesPerDayInIntervalForm::addCurrentConstraint);
	connect(addConstraintsPushButton, &QPushButton::clicked, this, &AddConstraintTeachersMaxRoomChangesPerDayInIntervalForm::addCurrentConstraints);
	connect(closePushButton, &QPushButton::clicked, this, &AddConstraintTeachersMaxRoomChangesPerDayInIntervalForm::close);
...
AddConstraintTeachersMaxRoomChangesPerDayInIntervalForm::~AddConstraintTeachersMaxRoomChangesPerDayInIntervalForm()
{
	saveFETDialogGeometry(this);
}

void AddConstraintTeachersMaxRoomChangesPerDayInIntervalForm::addCurrentConstraint()
{
	...
	ctr=new ConstraintTeachersMaxRoomChangesPerDayInInterval(weight, maxChangesSpinBox->value(), startHour, endHour);
...
void AddConstraintTeachersMaxRoomChangesPerDayInIntervalForm::addCurrentConstraints()
{
	...
	for(Teacher* tch : std::as_const(gt.rules.teachersList)){
		SpaceConstraint *ctr=new ConstraintTeacherMaxRoomChangesPerDayInInterval(weight, tch->name, maxChangesSpinBox->value(), startHour, endHour);
		bool tmp2=gt.rules.addSpaceConstraint(ctr);
		assert(tmp2);
