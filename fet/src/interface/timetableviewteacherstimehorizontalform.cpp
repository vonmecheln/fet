...
TimetableViewTeachersTimeHorizontalForm::TimetableViewTeachersTimeHorizontalForm(QWidget* parent): QDialog(parent)
{
	...
	teachersTimeAction=new QAction(tr("Teachers time", "View/edit the selected teachers' time constraints"), this);
	teachersSpaceAction=new QAction(tr("Teachers space", "View/edit the selected teachers' space constraints"), this);
	...
	constraintsMenu->addAction(teachersTimeAction);
	constraintsMenu->addAction(teachersSpaceAction);
	...
	connect(teachersTimeAction, &QAction::triggered, this, &TimetableViewTeachersTimeHorizontalForm::teachersTime);
	connect(teachersSpaceAction, &QAction::triggered, this, &TimetableViewTeachersTimeHorizontalForm::teachersSpace);
	connect(activitiesTimeAction, &QAction::triggered, this, &TimetableViewTeachersTimeHorizontalForm::activitiesTime);
	connect(activitiesSpaceAction, &QAction::triggered, this, &TimetableViewTeachersTimeHorizontalForm::activitiesSpace);
	connect(constraintsPushButton, &QPushButton::clicked, this, &TimetableViewTeachersTimeHorizontalForm::constraints);

	connect(helpPushButton, &QPushButton::clicked, this, &TimetableViewTeachersTimeHorizontalForm::help);

	connect(lockToDaysPushButton, &QPushButton::clicked, this, &TimetableViewTeachersTimeHorizontalForm::lockDays);
	connect(unlockFromDaysPushButton, &QPushButton::clicked, this, &TimetableViewTeachersTimeHorizontalForm::unlockDays);
	connect(unlockAllFromDaysPushButton, &QPushButton::clicked, this, &TimetableViewTeachersTimeHorizontalForm::unlockAllDays);
	...
	if(normalView){
		...
		for(int d=0; d<gt.rules.nRealDaysPerWeek; d++){
			...
			for(int h=0; h<gt.rules.nRealHoursPerDay; h++){
				...
				teachersTimetableTable->setHorizontalHeaderItem(d*gt.rules.nRealHoursPerDay+h, item);
			}
		}
	}
...
//		w=2*initialRecommendedHeight;
	...
	connect(widthSpinBox, qOverload<int>(&QSpinBox::valueChanged), this, &TimetableViewTeachersTimeHorizontalForm::widthSpinBoxValueChanged);
	connect(heightSpinBox, qOverload<int>(&QSpinBox::valueChanged), this, &TimetableViewTeachersTimeHorizontalForm::heightSpinBoxValueChanged);
...
void TimetableViewTeachersTimeHorizontalForm::newTimetableGenerated()
{
	//setupUi(this);
...
///////////
	...
	newItemDelegate->nRows=teachersTimetableTable->rowCount();
	bool realView=(gt.rules.mode==MORNINGS_AFTERNOONS && REAL_VIEW==true);
	if(!realView)
...
//	teachersTimetableTable->horizontalHeader()->setDefaultSectionSize(w);
	...
	connect(subjectsCheckBox, SIG NAL(toggled(bool)), this, SL OT(updateTeachersTimetableTable()));
	connect(studentsCheckBox, SIG NAL(toggled(bool)), this, SL OT(updateTeachersTimetableTable()));*/
	...
	for(int t=0; t<gt.rules.nInternalTeachers; t++){
		...
		for(int d=0; d<gt.rules.nDaysPerWeek; d++){
			for(int h=0; h<gt.rules.nHoursPerDay; h++){
				...
				// add colors (start)
				//if(USE_GUI_COLORS) {
				teachersTimetableTable->item(t, d*gt.rules.nHoursPerDay+h)->setBackground(teachersTimetableTable->palette().base());
				teachersTimetableTable->item(t, d*gt.rules.nHoursPerDay+h)->setForeground(teachersTimetableTable->palette().text());
				//}
				// add colors (end)
				//end by Marco Vassura
				...
				if(ai!=UNALLOCATED_ACTIVITY){
					...
					if(colorsCheckBox->isChecked() /*USE_GUI_COLORS*/ /*&& act->studentsNames.count()>0*/){
						...
						if (brightness<0.5)
							teachersTimetableTable->item(t, d*gt.rules.nHoursPerDay+h)->setForeground(QBrush(QColorConstants::White));
						else
							teachersTimetableTable->item(t, d*gt.rules.nHoursPerDay+h)->setForeground(QBrush(QColorConstants::Black));
					}
					// add colors (end)
					//end by Marco Vassura
...
QColor TimetableViewTeachersTimeHorizontalForm::stringToColor(const QString& s)
{
...
void TimetableViewTeachersTimeHorizontalForm::lock(bool lockTime, bool lockSpace)
{
	...
	if(addedT>0 || addedS>0 || unlockedT>0 || unlockedS>0)
		gt.rules.addUndoPoint(tr("Locked/unlocked/toggled a selection of activities in the"
		  " timetable view teachers time horizontal dialog. The summary of the added/removed locking constraints is:\n\n%1").arg(s)+QString("\n"));
...
void TimetableViewTeachersTimeHorizontalForm::unlockDays()
{
	...
	if(nUnlocked>0)
		gt.rules.addUndoPoint(tr("Unlocked from days a selection of activities in the"
		  " timetable view teachers time horizontal dialog. There were removed %1 locking constraints.").arg(nUnlocked)+QString("\n"));
...
void TimetableViewTeachersTimeHorizontalForm::lockDays()
{
	...
	if(nLocked>0)
		gt.rules.addUndoPoint(tr("Locked to days a selection of activities in the"
		  " timetable view teachers time horizontal dialog. There were added %1 locking time constraints.").arg(nLocked)+QString("\n"));
...
void TimetableViewTeachersTimeHorizontalForm::constraints()
{
	constraintsMenu->popup(QCursor::pos());
}

void TimetableViewTeachersTimeHorizontalForm::teachersTime()
{
	if(!(students_schedule_ready && teachers_schedule_ready)){
		QMessageBox::warning(this, tr("FET warning"), tr("Timetable not available in view teachers timetable dialog - please generate a new timetable"));
		return;
	...
	for(int t=0; t<gt.rules.nInternalTeachers; t++){
		...
		for(int k=0; k<teachersTimetableTable->columnCount(); k++){
			if(teachersTimetableTable->item(t, k)->isSelected()){
				stch.append(t);
				break;
			}
		}
	...
	if(stch.isEmpty()){
		QMessageBox::information(this, tr("FET information"), tr("Please select at least one teacher (one table cell)"));
		return;
	}

	QList<TimeConstraint*> ttcl;
	for(TimeConstraint* tc : std::as_const(gt.rules.timeConstraintsList)){
		for(int tch : std::as_const(stch)){
			if(tc->isRelatedToTeacher(gt.rules.internalTeachersList[tch]->name)){
				ttcl.append(tc);
				break;
			}
		}
	...
	QString s;
	if(stch.count()==1)
		s=tr("Teacher: %1").arg(gt.rules.internalTeachersList[stch.at(0)]->name);
	else
		s=tr("%1 teachers selected", "%1 is the number of selected teachers").arg(stch.count());
	ListOfRelatedTimeConstraintsForm form(this, FILTER_IS_TEACHER, QList<int>(), s, ttcl);
	form.exec();
}

void TimetableViewTeachersTimeHorizontalForm::teachersSpace()
{
	if(!(students_schedule_ready && teachers_schedule_ready)){
		QMessageBox::warning(this, tr("FET warning"), tr("Timetable not available in view teachers timetable dialog - please generate a new timetable"));
		return;
	...
	for(SpaceConstraint* sc : std::as_const(gt.rules.spaceConstraintsList)){
		for(int tch : std::as_const(stch)){
			if(sc->isRelatedToTeacher(gt.rules.internalTeachersList[tch]->name)){
				tscl.append(sc);
				break;
			}
		}
	...
	if(stch.count()==1)
		...
		s=tr("%1 teachers selected", "%1 is the number of selected teachers").arg(stch.count());
	ListOfRelatedSpaceConstraintsForm form(this, FILTER_IS_TEACHER, QList<int>(), s, tscl);
	form.exec();
}

void TimetableViewTeachersTimeHorizontalForm::activitiesTime()
{
	if(!(students_schedule_ready && teachers_schedule_ready)){
		QMessageBox::warning(this, tr("FET warning"), tr("Timetable not available in view teachers timetable dialog - please generate a new timetable"));
		return;
...
void TimetableViewTeachersTimeHorizontalForm::activitiesSpace()
{
	if(!(students_schedule_ready && teachers_schedule_ready)){
		QMessageBox::warning(this, tr("FET warning"), tr("Timetable not available in view teachers timetable dialog - please generate a new timetable"));
		return;
